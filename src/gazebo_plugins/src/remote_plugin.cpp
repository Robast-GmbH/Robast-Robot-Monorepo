...
: impl_(std::make_unique<RemotePluginPrivate>())
{
}
...
void RemotePlugin::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  impl_->model_ = _model;
  ...
  impl_->cmd_vel_sub_ = impl_->ros_node_->create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&RemotePluginPrivate::OnCmdVel, impl_.get(), std::placeholders::_1));
  ...
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&RemotePluginPrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}

void RemotePlugin::Reset()
{
  impl_->last_update_time_ = impl_->world_->SimTime();
...
void RemotePluginPrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  double seconds_since_last_update = (_info.simTime - last_update_time_).Double();

  std::lock_guard<std::mutex> scoped_lock(lock_);
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("RemotePluginPrivate::OnUpdate");
...
void RemotePluginPrivate::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
{
  std::lock_guard<std::mutex> scoped_lock(lock_);
  target_cmd_vel_ = *_msg;
}

void RemotePluginPrivate::UpdateOdometry(const gazebo::common::Time & _current_time)
{
...
void RemotePluginPrivate::PublishOdometryTf(const gazebo::common::Time & _current_time)
{
...
GZ_REGISTER_MODEL_PLUGIN(RemotePlugin)
}  // namespace gazebo_plugins