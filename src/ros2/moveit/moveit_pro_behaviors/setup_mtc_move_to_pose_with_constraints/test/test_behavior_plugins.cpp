...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("setup_mtc_move_to_pose_with_constraints::SetupMtcMoveToPoseWithConstraintsBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "SetupMtcMoveToPoseWithConstraints", BT::NodeConfiguration()));
}
