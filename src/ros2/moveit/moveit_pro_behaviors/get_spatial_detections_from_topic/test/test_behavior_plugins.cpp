...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("get_spatial_detections_from_topic::GetSpatialDetectionsFromTopicBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "GetSpatialDetectionsFromTopic", BT::NodeConfiguration()));
}
