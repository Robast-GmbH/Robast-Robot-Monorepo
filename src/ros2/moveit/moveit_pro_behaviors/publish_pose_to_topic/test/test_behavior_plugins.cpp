...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("publish_pose_to_topic::PublishPoseToTopicBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "PublishPoseToTopic", BT::NodeConfiguration()));
}
