...
namespace create_pose_from_spatial_detections
{
  ...
  constexpr auto PORT_ID_INVERT_Y_AXIS = "invert_y_axis";
  ...
  BT::PortsList CreatePoseFromSpatialDetections::providedPorts()
  {
    return BT::PortsList({
      ...
      BT::InputPort<depthai_ros_msgs::msg::SpatialDetectionArray>(
        PORT_ID_SPATIAL_DETECTIONS, "{spatial_detections}", "The spatial detections to create a pose from."),
      BT::InputPort<bool>(PORT_ID_INVERT_Y_AXIS, false, "Whether to invert the y-axis of the position of the pose."),
      BT::InputPort<std::vector<double>>(PORT_ID_ORIENTATION,
                                         {0.0, 0.0, 1.57079632679},
                                         "The target orientation of the pose (optional) in euler (rad)."),
      BT::OutputPort<geometry_msgs::msg::PoseStamped>(
        PORT_ID_POSE_STAMPED_OUT, "{pose_stamped}", "The output pose stamped."),
    });
  }
  ...
  {
    return {{"description",
             "Creates a pose from given spatial detections."
  ...
  tl::expected<bool, std::string> CreatePoseFromSpatialDetections::doWork()
  {
    ...
    const auto invert_y_axis = getInput<bool>(PORT_ID_INVERT_Y_AXIS);

    // Check that all required input data ports were set
    if (const auto error = moveit_studio::behaviors::maybe_error(confidence_threshold,
                                                                 spatial_detections,
    ...
    depthai_ros_msgs::msg::SpatialDetection highest_confidence_detection = *std::max_element(
      spatial_detections.value().detections.begin(),
      spatial_detections.value().detections.end(),
    ...
    if (use_confidence_threshold.value() &&
        (highest_confidence_detection.results[0].score < confidence_threshold.value()))
    {
    ...
    pose_stamped.pose.position.y =
      invert_y_axis.value() ? (-1) * highest_confidence_detection.position.y : highest_confidence_detection.position.y;
    ...
    {
      tf2::Quaternion q;
      q.setRPY(target_orientation_in_euler.value()[0],
               target_orientation_in_euler.value()[1],
               target_orientation_in_euler.value()[2]);
