...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("create_pose_from_spatial_detections::CreatePoseFromSpatialDetectionsBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "CreatePoseFromSpatialDetections", BT::NodeConfiguration()));
}
