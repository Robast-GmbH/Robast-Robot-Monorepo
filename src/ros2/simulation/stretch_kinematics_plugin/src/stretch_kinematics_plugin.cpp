...
namespace stretch_kinematics_plugin
{
                                           ...
                                           double search_discretization)
  {
    ...
    if (const auto group_it = std::find_if(sub_groups.cbegin(),
                                           sub_groups.cend(),
    ...
    if (auto group_it = std::find_if(sub_groups.cbegin(),
                                     sub_groups.cend(),
                                     [](const moveit::core::JointModelGroup* const group)
                                     {
    ...
    else
    {
      RCLCPP_ERROR_STREAM(LOGGER,
                          "Failed to find a sub-group for the mobile base in " << joint_model_group_->getName());
      return false;
    }
    ...
    {
      RCLCPP_ERROR(
          LOGGER, "Could not find tip name '%s' in joint group '%s'", getTipFrame().c_str(), group_name.c_str());
      return false;
    }
                                              ...
                                              const kinematics::KinematicsQueryOptions& options) const
  {
    ...
    return searchPositionIK(
        ik_pose, ik_seed_state, 0.0, consistency_limits, solution, IKCallbackFn(), error_code, options);
  }
                                                 ...
                                                 const kinematics::KinematicsQueryOptions& options) const
  {
    ...
    return searchPositionIK(
        ik_pose, ik_seed_state, timeout, consistency_limits, solution, IKCallbackFn(), error_code, options);
  }
                                                 ...
                                                 const kinematics::KinematicsQueryOptions& options) const
  {
    ...
    {
      RCLCPP_ERROR(LOGGER, "Seed state must have size %d instead of size %li\n", dimension_, ik_seed_state.size());
      error_code.val = moveit_msgs::msg::MoveItErrorCodes::NO_IK_SOLUTION;
      return false;
    }
    ...
    const auto arm_jmg_consistency_limits =
        consistency_limits.empty() ? std::vector<double>{}
                                   : std::vector<double>(consistency_limits.cbegin(),
                                                         std::next(consistency_limits.cbegin(), mobile_base_index_));
    ...
    do
    {
      ...
      // solution and call solution_callback
      ...
      if (bool(solution_callback))   // TODO: Check if this works. It was actually: (!(&solution_callback).empty())
      {
        arm_solution_callback =
            [&solution_callback, &solution = std::as_const(solution), mobile_base_index = this->mobile_base_index_](
                ...
                const std::vector<double>& arm_solution,
                moveit_msgs::msg::MoveItErrorCodes& error_code)
        {
      ...
      Eigen::Map<Eigen::Vector2d>(&solution.at(mobile_base_index_)) =
          T_mobile_base_link.translation().block<2, 1>(0, 0);
