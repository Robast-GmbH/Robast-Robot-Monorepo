...
FleetAdapter::FleetAdapter(const rclcpp::NodeOptions& options)
{
  ...
  _data->node = std::make_shared<rclcpp::Node>("turtlebot_fleet_adapter", options);
  ...
  RCLCPP_INFO(_data->node->get_logger(), "Starting %s", _data->node->get_name());

  _data->fleet_name = _data->node->declare_parameter("fleet_name", "turtlebot");
  RCLCPP_INFO(_data->node->get_logger(), "Configuring fleet [%s].", _data->fleet_name.c_str());

  _data->adapter = std::make_shared<rmf_fleet_adapter::agv::EasyFullControl>();

  if (_data->adapter != nullptr)
  {
    RCLCPP_INFO(_data->node->get_logger(), "Created EasyFullControl adapter instance.");
    add_robots();
  }
  else
  {
    RCLCPP_INFO(_data->node->get_logger(), "Failed to create EasyFullControl adapter instance.");
  }
}
//==============================================================================
void FleetAdapter::add_robots()
{
  auto add_robot = [data = _data](const std::string& ns,
                                  ...
                                  rclcpp::Node::SharedPtr node)
  {
    ...
    {
      RCLCPP_WARN(
          node->get_logger(), "Attempted to add robot with name [%s] more than once. Ignoring...", name.c_str());
      return;
    }
    auto robot = std::make_shared<Robot>();
    // This is a block call.
    RCLCPP_INFO(node->get_logger(), "Initializing robot [%s]", name.c_str());
    if (robot->initialize(ns, name, charger_name, initial_map_name, node))
    {
      ...
      data->adapter->add_robot(
          ...
          [robot = insertion.first->second](const std::string& map_name,
                                            const Eigen::Vector3d goal,
                                            RobotUpdateHandlePtr robot_handle) -> RobotCallbacks
          {
          ...
          // [robot = insertion.first->second](const std::string& dock_name,
          //                                   RobotUpdateHandlePtr robot_handle) -> RobotCallbacks
          // {
          //   return robot->dock(dock_name, robot_handle);
          // },
          [robot = insertion.first->second](const std::string& category,
    ...
    {
      RCLCPP_ERROR(node->get_logger(), "Failed to initialize robot [%s]", name.c_str());
    }
  };
  ...
  for (const auto& r : robots)
  {
    ...
    initial_map_names.push_back(_data->node->declare_parameter(r + ".initial_map_name", "L1"));
    charger_names.push_back(_data->node->declare_parameter(r + ".charger_waypoint", "tinyRobot1_charger"));
  }
  ...
  {
    _data->threads.push_back(
        std::thread(add_robot, namespaces[i], robots[i], charger_names[i], initial_map_names[i], _data->node));
  }
}

//==============================================================================
auto FleetAdapter::Robot::initialize(const std::string& ns,
                                     ...
                                     rclcpp::Node::SharedPtr node_) -> bool
{
  ...
  odom_sub = node->create_subscription<Odom>(
      ...
      [&](Odom::ConstSharedPtr msg)
      {
        ...
        location = Eigen::Vector3d{msg->pose.pose.position.x, msg->pose.pose.position.y, yaw};
  ...
  nav2_client = rclcpp_action::create_client<NavigationAction>(node, ns + "/navigate_to_pose");
...
auto FleetAdapter::Robot::get_state() -> RobotState
{
  ...
  auto state = RobotState(map_name, l, battery_soc);

  return state;
}

//==============================================================================
auto FleetAdapter::Robot::navigate(const std::string& map_name,
                                   const Eigen::Vector3d goal_,
                                   RobotUpdateHandlePtr robot_handle) -> RobotCallbacks
{
  auto cb = [this](rmf_traffic::Duration& remaining_time_, bool& request_replan_) -> bool
  {
  ...
  auto goal_options = rclcpp_action::Client<NavigationAction>::SendGoalOptions();
  goal_options.goal_response_callback = [this](const GoalHandle::SharedPtr& goal_handle)
  {
    if (!goal_handle)
    {
      RCLCPP_ERROR(node->get_logger(), "Navigation goal was rejected by server for robot [%s]", name.c_str());
    }
    else
    {
      RCLCPP_INFO(node->get_logger(), "Navigation goal accepted by server for robot [%s].", name.c_str());
    }
  };
  goal_options.feedback_callback =
      [this](GoalHandle::SharedPtr, const std::shared_ptr<const NavigationAction::Feedback> feedback)
  {
    // remaining_time =
    //   rmf_traffic_ros2::convert(feedback->estimated_time_remaining);
  };
  goal_options.result_callback = [this](const GoalHandle::WrappedResult& result)
  {
    switch (result.code)
    {
...
//==============================================================================
auto FleetAdapter::Robot::dock(const std::string& dock_name, RobotUpdateHandlePtr robot_handle) -> GoalCompletedCallback
{
  auto cb = [this](rmf_traffic::Duration& remaining_time_, bool& request_replan_) -> bool
  {
    return true;
...
//==============================================================================
void FleetAdapter::Robot::action_executor(const std::string& category,
                                          const nlohmann::json& description,
                                          RobotUpdateHandle::ActionExecution execution)
{
  execution.finished();
