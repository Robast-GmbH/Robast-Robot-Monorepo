...
namespace door_manipulator_gate
{
    DoorManipulatorGate::DoorManipulatorGate(const string serial_path): Node("door_manipulator_gate")
    {
        ...
        this->norelem_stepper_control_subscription_ = this->create_subscription<NorelemStepperControl>(
            "control_norelem_stepper", qos, std::bind(&DoorManipulatorGate::control_norelem_stepper_topic_callback, this, std::placeholders::_1));

        this->send_ascii_cmds_timer_ = this->create_wall_timer(TIMER_PERIOD_SEND_ASCII_CMDS, std::bind(&DoorManipulatorGate::send_ascii_cmds_timer_callback, this));
    ...
    void DoorManipulatorGate::control_norelem_stepper_topic_callback(const NorelemStepperControl& msg)
    {
        ...
        robast_can_msgs::CanMessage can_msg_door_manipulator = this->create_can_msg_norelem_stepper(drawer_controller_id, norelem_stepper_parameter);

        this->send_can_msg(can_msg_door_manipulator);
    }

    robast_can_msgs::CanMessage DoorManipulatorGate::create_can_msg_norelem_stepper(uint32_t drawer_controller_id, norelem_stepper_parameter norelem_stepper_parameter) const
    {
