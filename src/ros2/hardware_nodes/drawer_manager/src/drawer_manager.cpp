...
namespace nfc_gate
{
  DrawerManager::DrawerManager() : Node("drawer_manager_Node")
  {
    ...
    this->drawers_info_server = this->create_service<ShelfSetupInfo>(
        "get_module_setup",
        std::bind(&DrawerManager::get_shelf_setup, this, std::placeholders::_1, std::placeholders::_2));
    this->drawers_info_client =
        this->create_client<ShelfSetupInfo>("shelf_setup_info", rmw_qos_profile_services_default, callback_group);
    ...
    this->drawer_interaction_server = rclcpp_action::create_server<DrawerInteraction>(
        this,
        "drawer_interaction",
        bind(&DrawerManager::drawer_interaction_goal_callback, this, std::placeholders::_1, std::placeholders::_2),
        bind(&DrawerManager::drawer_interaction_cancel_callback, this, std::placeholders::_1),
        bind(&DrawerManager::drawer_interaction_accepted_callback, this, std::placeholders::_1));

    RCLCPP_INFO(this->get_logger(), "Finished Creating");
  }

  void DrawerManager::get_shelf_setup(const std::shared_ptr<ShelfSetupInfo::Request> request,
                                      std::shared_ptr<ShelfSetupInfo::Response> response)
  {
    // Wait for the service to be alive.
    while (!drawers_info_client->wait_for_service(std::chrono::seconds(1)))
    {
      if (!rclcpp::ok())
      {
  ...
  rclcpp_action::GoalResponse DrawerManager::drawer_interaction_goal_callback(
      const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const DrawerInteraction::Goal> goal)
  {
    (void) uuid;
    (void) goal;
  ...
  rclcpp_action::CancelResponse DrawerManager::drawer_interaction_cancel_callback(
      const std::shared_ptr<GoalHandleDrawerInteraction> goal_handle)
  {
    (void) goal_handle;
  ...
  void DrawerManager::drawer_interaction_accepted_callback(
      const std::shared_ptr<GoalHandleDrawerInteraction> goal_handle)
  {
    RCLCPP_INFO(this->get_logger(), "open drawer");   // DEBUGGING
    RCLCPP_INFO(this->get_logger(),
                "authentication_result_callback %s",
                goal_handle->get_goal().get()->task.ticket.load_keys[0].c_str());   // DEBUGGING
  ...
  {
    const std::shared_ptr<const communication_interfaces::action::DrawerInteraction_Goal> goal =
  ...
  void DrawerManager::drawer_interaction_state_machine(
      const std::shared_ptr<const communication_interfaces::action::DrawerInteraction_Goal> goal, uint8_t state)
  {
    ...
    switch (state)
    {
      case 1:
      {
        ...
        DrawerManager::AuthenticateUserResultHandle user_id_action_handle = this->request_user_authentication(
            goal->loading, goal->task.ticket.load_keys, goal->task.ticket.drop_of_keys);
      ...
      case 2:
      {
        ...
        DrawerManager::DrawerUserAccessResultHandle drawer_user_access_action_handle =
            this->request_drawer_user_access(goal->task.drawer_address);
  ...
  DrawerManager::AuthenticateUserResultHandle DrawerManager::request_user_authentication(
      bool loading, std::vector<std::string> load_keys, std::vector<std::string> drop_of_keys)
  {
    // NFC Reader
    ...
    send_goal_options.goal_response_callback =
        std::bind(&DrawerManager::authentication_goal_response_callback, this, std::placeholders::_1);
    send_goal_options.feedback_callback =
        std::bind(&DrawerManager::authentication_feedback_callback, this, std::placeholders::_1, std::placeholders::_2);

    return this->authenticate_user_client->async_send_goal(authentication_request, send_goal_options);
  }

  std::string DrawerManager::wait_for_user_authentication(AuthenticateUserResultHandle action_handle)
  {
  ...
  void DrawerManager::authentication_feedback_callback(GoalHandleAuthenticateUser::SharedPtr,
                                                       const std::shared_ptr<const AuthenticateUser::Feedback> feedback)
  {
  ...
  DrawerManager::DrawerUserAccessResultHandle DrawerManager::request_drawer_user_access(DrawerAddress drawer_address)
  {
    ...
    send_goal_options.goal_response_callback =
        std::bind(&DrawerManager::open_drawer_goal_response_callback, this, std::placeholders::_1);
    send_goal_options.feedback_callback =
        std::bind(&DrawerManager::open_drawer_feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
  ...
  void DrawerManager::wait_for_finished_drawer_user_access(
      DrawerUserAccessResultHandle drawer_user_access_action_handle)
  {
  ...
  void DrawerManager::ask_user_for_reopening_drawer(
      const std::shared_ptr<const communication_interfaces::action::DrawerInteraction_Goal> goal)
  {
  ...
  void DrawerManager::open_drawer_feedback_callback(GoalHandleDrawerUserAccess::SharedPtr,
                                                    const std::shared_ptr<const DrawerUserAccess::Feedback> feedback)
  {
  ...
  {
    RCLCPP_INFO(this->get_logger(),
                "UserInterface::Bitte schließen sie die Schublade nachdem sie alles entnommen haben.");
  }
