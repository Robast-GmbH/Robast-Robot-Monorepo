...
namespace drawer_bridge
{
  ...
  void CanSender::add_can_message_to_queue(can_msgs::msg::Frame can_msg)
  {
    ...
    // // if queue was empty, the timer has been canceled before, so start timer for timer callbacks which trigger
    // sending
    // // the ascii cmds
    ...
    //   {
    //     // In case the queue gets to large, it can happen that some can messages wont be published to the bus
    //     properly
    ...
    //       // TODO: How do i just change the interval of the timer? For cancel and restart it, but this might go
    //       easier _send_can_msgs_timer->cancel(); _send_can_msgs_timer =
