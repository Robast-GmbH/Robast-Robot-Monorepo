...
{
  TestNFCBridge::TestNFCBridge(serial_helper::MockSerialHelper* serial_connector,
                               db_helper::MockPostgreSqlHelper* db_connector)
      : NFCBridge()
  {
    NFCBridge::_serial_connector = std::make_unique<serial_helper::MockSerialHelper>(*serial_connector);
    NFCBridge::_db_connector = std::make_unique<db_helper::MockPostgreSqlHelper>(*db_connector);
  }
  ...
  TestNFCBridge::TestNFCBridge() : NFCBridge()
  {
    ...
    NFCBridge::_serial_connector = std::make_unique<serial_helper::MockSerialHelper>(
        serial_helper::MockSerialHelper(serial_helper::MockSerialHelper(key)));

    NFCBridge::_db_connector =
  ...
  {
    return _db_connector->checkUserTag(scanned_key, related_username, related_id, std::shared_ptr<std::string>());
  }

  bool TestNFCBridge::read_nfc_code(std::shared_ptr<std::string> scanned_key)
  {
    return _serial_connector->read_serial(scanned_key.get(), 50);
  }
