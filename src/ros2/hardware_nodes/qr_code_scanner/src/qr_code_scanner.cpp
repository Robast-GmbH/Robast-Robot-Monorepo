...
void QrCodeScanner::toggle_image_subscription(bool enable)
{
  if (enable)
  {
  ...
  std::shared_ptr<communication_interfaces::srv::ReadQrCode::Response> response)
{
  ...
  {
    toggle_image_subscription(true);
  }
  ...
  const int timeout_limit = request->timeout_in_s * 10;
  ...
  if (_active_service_calls_counter == 0)
  {
    toggle_image_subscription(false);
  }
}
...
std::vector<std::string> QrCodeScanner::read_qr_codes_from_image(const cv::Mat &image)
{
  ...
  const int num_codes = quirc_count(_qr_decoder);
