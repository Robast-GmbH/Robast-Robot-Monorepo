...
std::tuple<dai::Pipeline, int, int> createPipeline(const bool lrcheck,
                                                   const bool extended,
                                                   const bool subpixel,
                                                   const bool rectify,
...
void getParameters(rclcpp::Node::SharedPtr node,
                   ...
                   bool &syncNN,
                   bool &enableDotProjector,
                   bool &enableFloodLight,
                   double &dotProjectormA,
                   double &floodLightmA,
...
int main(int argc, char **argv)
{
  ...
  auto depthCameraInfo = depth_aligned ? rgbConverter.calibrationToCameraInfo(
                                           calibrationHandler, dai::CameraBoardSocket::CAM_A, width, height)
                                       : rightCameraInfo;
  ...
  auto previewCameraInfo = rgbConverter.calibrationToCameraInfo(
    calibrationHandler, dai::CameraBoardSocket::CAM_A, previewWidth, previewHeight);

  dai::rosBridge::SpatialDetectionConverter detConverter(tfPrefix + "_rgb_camera_optical_frame", 416, 416, false);
  dai::rosBridge::BridgePublisher<depthai_ros_msgs::msg::SpatialDetectionArray, dai::SpatialImgDetections>
