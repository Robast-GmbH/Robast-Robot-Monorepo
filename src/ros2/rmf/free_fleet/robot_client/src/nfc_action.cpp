...
NFCAction::NFCAction(int task_id, int step, std::shared_ptr<rclcpp::Node> ros_node, int user_id)
: Action(task_id, step, ros_node)
{
  this->user_id = user_id;
  nfc_write_new_nfc_card_client_ = rclcpp_action::create_client<CreateUserNfcTag>(
    ros_node, ros_node_->get_parameter(
      "nfc_write_usertag_to_card_action_topic").as_string());
}
...
void NFCAction::start_writing_procedure()
{
  ...
  send_goal_options.goal_response_callback = std::bind(
    &NFCAction::goal_response_callback, this,
    std::placeholders::_1);
  send_goal_options.feedback_callback = std::bind(
    &NFCAction::feedback_callback, this,
    std::placeholders::_1, std::placeholders::_2);
  send_goal_options.result_callback = std::bind(
    &NFCAction::result_callback, this,
    std::placeholders::_1);
...
void NFCAction::feedback_callback(
  GoalHandleCreateUserNfcTag::SharedPtr,
