...
: Node("robot_client")
{
  ...
  reset_simple_tree_publisher_ =
    this->create_publisher<StdMsgBool>(
    this->get_parameter(
      "statemaschine_reset_simple_tree_topic").as_string(), qos_statemaschine);
}
...
void RobotClient::init_param()
{
  ...
  this->declare_parameter(
    "behavior_tree",
    "/workspace/src/navigation/nav_bringup/behavior_trees/humble/navigate_to_pose_w_replanning_goal_patience_and_recovery.xml");
...
void RobotClient::start_receive_tasks()
{
  ...
  write_nfc_card_request_subscriber_ = this->create_subscription<FreeFleetDataCreateNfcRequest>(
    this->get_parameter(
      "fleet_communication_create_nfc_topic").as_string(), qos,
  ...
  drawer_request_subscriber_ = this->create_subscription<FreeFleetDataDrawerRequest>(
    this->get_parameter(
      "fleet_communication_drawer_topic").as_string(), qos,
  ...
  navigation_request_subscriber_ = this->create_subscription<FreeFleetDataDestinationRequest>(
    this->get_parameter(
      "fleet_communication_destination_topic").as_string(), qos,
    std::bind(&RobotClient::receive_destination_task, this, std::placeholders::_1));
  setting_subscriber_ =
    this->create_subscription<FreeFleetDataSettingRequest>(
    this->get_parameter(
      "fleet_communication_setting_topic").as_string(), qos,
    std::bind(&RobotClient::receive_settings, this, std::placeholders::_1));
  drawer_status_subscriber_ =
    this->create_subscription<DrawerStatus>(
    this->get_parameter(
      "drawer_status_change_topic").as_string(), 10,
    std::bind(&RobotClient::receive_drawer_status, this, std::placeholders::_1));
  authentication_subscriber_ =
    this->create_subscription<StdMsgInt>(
    this->get_parameter(
      "nfc_authenticated_user_topic").as_string(), 10,
...
void RobotClient::start_update_robot_state()
{
  ...
  robot_info_publisher_ =
    ...
    this->get_parameter(
      "fleet_communication_robot_info_topic").as_string(), qos_fleet_communication);
  publish_robot_info_timer_ = this->create_wall_timer(
    std::chrono::seconds(
      robot_info_inteval), std::bind(&RobotClient::publish_fleet_state, this));
}
...
void RobotClient::receive_create_nfc_task(const FreeFleetDataCreateNfcRequest::ConstPtr msg)
{
  ...
  if (!task_sequence.insert(
      std::make_pair(
        step,
        std::make_unique<NFCAction>(task_id, step, shared_from_this(), msg->user_id))).second)
  {
    RCLCPP_ERROR(
      this->get_logger(), "Task %i, step %i could not be added to robot Queue", task_id, step);
    return;
  }
  if (step == 1) {
  ...
  if (!task_sequence.insert(
      std::make_pair(
        ...
        std::make_unique<NavigationAction>(
          task_id, step, shared_from_this(), msg->destination.x,
          msg->destination.y, msg->destination.yaw))).second)
  {
    RCLCPP_ERROR(
      this->get_logger(), "Task %i, step %i could not be added to robot Queue", task_id, step);
...
void RobotClient::receive_drawer_task(const FreeFleetDataDrawerRequest::ConstPtr msg)
{
  ...
  if (!task_sequence.insert(
      std::make_pair(
        step,
        std::make_unique<DrawerAction>(
          DrawerAction(
            task_id, step, shared_from_this(), drawer_list,
            msg->drawer_id, msg->module_id, msg->e_drawer, msg->authorized_user)))).second)
  {
    RCLCPP_ERROR(
      this->get_logger(), "Task %i, step %i could not be added to robot Queue", task_id, step);
    return;
...
void RobotClient::receive_drawer_status(const DrawerStatus::SharedPtr msg)
{
  ...
  if (current_step == 0 || current_step > task_sequence.size()) {
    ...
    task_sequence[current_step]->receive_new_settings(
      "DrawerState",
      {drawer_ref + "#" + (msg->drawer_is_open ? "Opened" : "Closed")});
  }
...
bool RobotClient::prepare_new_action(
  std::string task_def, std::string recipient_fleet,
  std::string recipient_robot, int & new_task_id, int & new_step)
{
