...
class free_fleet_client_direct(Node):
    def __init__(self):
        ...
        self.dds_domain = self.get_parameter('dds_domain')\
                              .get_parameter_value()\
                              .integer_value

        self.dds_open_drawer_topic = self.get_parameter('dds_open_drawer_topic') \
                                         .get_parameter_value()\
                                         .string_value
    ...
    def start_receiving_from_dds(self):
        open_drawer = Thread(target=self.dds_subscriber,
                             args=(self.dds_domain,
                                   self.dds_open_drawer_topic,
                                   messages.FreeFleetData_OpenDrawerRequest,
                                   self.ros_publish_drawer_open), daemon=True)
        open_drawer.start()
