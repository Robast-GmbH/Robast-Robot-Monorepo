...
BOUNCE_TIME_IN_MS = 300 # the switch of the disinfection module closes for 300ms when triggered
...
class DisinfectionPublisher(Node):
    ...
    def __get_parameter(self):
        self.__read_in_disinfection_switch_counter_limit = self.get_parameter("read_in_disinfection_switch_counter_limit").get_parameter_value().integer_value
        self.__disinfection_switch_threshold = self.get_parameter("disinfection_switch_threshold").get_parameter_value().double_value
        self.__timer_period_in_sec = self.get_parameter("disinfection_timer_period_in_sec").get_parameter_value().double_value
    ...
    def __trigger_disinfection_evaluation(self, channel: int):
        self.__read_in_disinfection_switch_counter = 0
        self.__running_average_disinfection_switch = 0
        self.get_logger().info(f"Disinfection switch triggered on channel {channel}. Evaluate disinfection switch for {self.__read_in_disinfection_switch_counter_limit} times.")
        self.__timer = self.create_timer(self.__timer_period_in_sec, self.__evaluate_disinfection_switch)

    def __evaluate_disinfection_switch(self):
        self.__read_in_disinfection_switch_pin()
        if self.__read_in_disinfection_switch_counter >= self.__read_in_disinfection_switch_counter_limit:
            self.__timer.cancel()
            if self.__running_average_disinfection_switch >= self.__disinfection_switch_threshold:
                self.__publish_disinfection_triggered_with_stamp()
            else:
                self.get_logger().info(f"False positive detected with running average: {self.__running_average_disinfection_switch}")
    ...
    def __read_in_disinfection_switch_pin(self):
        ...
        self.__running_average_disinfection_switch = (
            self.__running_average_disinfection_switch * 0.9 + current_disinfection_switch * 0.1
        )
