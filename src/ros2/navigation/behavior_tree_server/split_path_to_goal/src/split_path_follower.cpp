#include "split_path_follower.hpp"
...
namespace split_path_follower
{
  ...
  nav2_util::CallbackReturn SplitPathFollower::on_configure(const rclcpp_lifecycle::State& /*state*/)
  {
    ...
    nav_to_pose_client_ =
      rclcpp_action::create_client<ClientT>(get_node_base_interface(),
                                            get_node_graph_interface(),
                                            get_node_logging_interface(),
                                            get_node_waitables_interface(),
                                            "navigate_to_pose",
                                            callback_group_);   // TODO hier doch eig waypoint follower

    action_server_ = std::make_unique<ActionServer>(get_node_base_interface(),
                                                    get_node_clock_interface(),
                                                    get_node_logging_interface(),
                                                    get_node_waitables_interface(),
                                                    "split_path_waypoints",
                                                    std::bind(&SplitPathFollower::followWaypoints, this));

    try
    {
      waypoint_task_executor_type_ = nav2_util::get_plugin_type_param(this, waypoint_task_executor_id_);
      ...
      RCLCPP_INFO(get_logger(),
                  ...
                  waypoint_task_executor_id_.c_str(),
  ...
  nav2_util::CallbackReturn SplitPathFollower::on_activate(const rclcpp_lifecycle::State& /*state*/)
  {
    ...
    dyn_params_handler_ =
      node->add_on_set_parameters_callback(std::bind(&SplitPathFollower::dynamicParametersCallback, this, _1));
  ...
  nav2_util::CallbackReturn SplitPathFollower::on_deactivate(const rclcpp_lifecycle::State& /*state*/)
  {
  ...
  nav2_util::CallbackReturn SplitPathFollower::on_cleanup(const rclcpp_lifecycle::State& /*state*/)
  {
  ...
  nav2_util::CallbackReturn SplitPathFollower::on_shutdown(const rclcpp_lifecycle::State& /*state*/)
  {
    RCLCPP_INFO(get_logger(), "Shutting down");
    return nav2_util::CallbackReturn::SUCCESS;
  }

  void SplitPathFollower::followWaypoints()
  {
    ...
    RCLCPP_INFO(
      get_logger(), "Received follow waypoint request with %i waypoints.", static_cast<int>(goal->poses.size()));
    ...
    while (rclcpp::ok())
    {
      ...
      if (new_goal)
      {
        ...
        send_goal_options.result_callback = std::bind(&SplitPathFollower::resultCallback, this, std::placeholders::_1);
      ...
      else if (current_goal_status_ == ActionStatus::SUCCEEDED)
      {
        ...
        RCLCPP_INFO(
          get_logger(), "Task execution at waypoint %i %s", goal_index, is_task_executed ? "succeeded" : "failed!");
      ...
      if (current_goal_status_ != ActionStatus::PROCESSING && current_goal_status_ != ActionStatus::UNKNOWN)
      {
        ...
        {
          RCLCPP_INFO(get_logger(), "Completed all %lu waypoints requested.", goal->poses.size());
          result->missed_waypoints = failed_ids_;
          action_server_->succeeded_current(result);
          failed_ids_.clear();
          return;
  ...
  void SplitPathFollower::resultCallback(const rclcpp_action::ClientGoalHandle<ClientT>::WrappedResult& result)
  {
    switch (result.code)
    {
  ...
  void SplitPathFollower::goalResponseCallback(const rclcpp_action::ClientGoalHandle<ClientT>::SharedPtr& goal)
  {
    if (!goal)
    {
  ...
  rcl_interfaces::msg::SetParametersResult SplitPathFollower::dynamicParametersCallback(
