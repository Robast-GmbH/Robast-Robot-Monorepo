...
def generate_launch_description():
    ...
    declare_slam_toolbox_params_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            robast_nav_launch_dir, "config", "slam_toolbox_params_offline.yaml"
        ),
    ...
    declare_slam_base_launch_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            robast_nav_launch_dir, "launch", "slam_toolbox_base_launch.py"
        ),
    ...
    declare_slam_posegraph_file_cmd = DeclareLaunchArgument(
        "slam_posegraph",
        default_value=os.path.join(robast_nav_launch_dir, "maps", "5OG", "5OG"),
    ...
    slam_base_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(slam_base_launch_file),
        launch_arguments=slam_arguments,
    )

    amcl_base_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(amcl_base_launch_file),
        launch_arguments=amcl_arguments,
    )
