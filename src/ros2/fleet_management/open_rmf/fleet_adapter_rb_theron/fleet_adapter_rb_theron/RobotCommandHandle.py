    ...
    def follow_new_path(
        ...
        def _follow_path():
            ...
            while (
                ...
                or self.state == RobotState.WAITING
            ):
                ...
                if self.state == RobotState.IDLE:
                    ...
                    response = self.api.navigate(
                        self.name,
                        [target_pose[0], target_pose[1], theta],
                        self.map_name,
                    )
    ...
    def update_state(self):
        ...
        if not self.charger_is_set:
            if "max_delay" in self.config.keys():
                ...
                self.node.get_logger().info(f"Setting max delay to {max_delay}s")
        ...
        with self._lock:
            ...
            # if robot is merging into a waypoint
                ...
                self.update_handle.update_lost_position(self.map_name, self.position)
