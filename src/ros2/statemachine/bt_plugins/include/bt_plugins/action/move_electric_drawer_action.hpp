...
namespace statemachine
{
  ...
  {
   public:
    MoveElectricDrawer(const std::string &name, const BT::NodeConfig &config);
    ...
    static BT::PortsList providedPorts()
    {
      return {BT::InputPort<communication_interfaces::msg::DrawerAddress>(
              ...
              BT::InputPort<uint8_t>("target_position", 0, "goal position between 0-255"),
              BT::InputPort<uint8_t>("speed", 0, "speed between 0-255"),
              BT::InputPort<uint8_t>(
                  "stall_guard_value",
                  0,
                  "value between 0-255, 0 disables stall guard, higher value increases stall sensitivity"),
              BT::InputPort<std::string>(
                  "move_electric_drawer_topic", "/move_drawer", "topic thats used to execute the action")};
    }
