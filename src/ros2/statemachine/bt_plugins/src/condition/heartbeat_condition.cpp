...
namespace statemachine
{
  ...
  BT::NodeStatus HeartbeatCondition::tick()
  {
    ...
    // Loop through all the received heartbeats and check if any of them have timed out
    for (const auto &heartbeat : _last_heartbeat_timestamp_by_id)
    {
      const std::string &id = heartbeat.first;
      const rclcpp::Time &last_timestamp = heartbeat.second;
      const std::chrono::milliseconds time_since_last_heartbeat_in_ms =
          utils::convert_to_milliseconds(now) - utils::convert_to_milliseconds(last_timestamp);
  ...
  void HeartbeatCondition::_callback_heartbeat(const communication_interfaces::msg::Heartbeat::SharedPtr msg)
  {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("HeartbeatCondition"),
                 ...
                 msg->id.c_str(),
                 _heartbeat_interval_in_ms_by_id,
