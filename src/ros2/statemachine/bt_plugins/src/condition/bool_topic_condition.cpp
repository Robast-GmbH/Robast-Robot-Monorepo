...
{
  BoolTopicCondition::BoolTopicCondition(const std::string &name, const BT::NodeConfig &config)
      : BaseCompareCondition(name, config, rclcpp::QoS(rclcpp::KeepLast(2)).best_effort())
  {
  ...
  bool BoolTopicCondition::comparator(std_msgs::msg::Bool last_message, bool target_value)
  {
    if ((last_message.data == target_value))
    {
      RCLCPP_DEBUG(rclcpp::get_logger("BoolTopicCondition"), "value received: %d", last_message.data);
      RCLCPP_DEBUG(rclcpp::get_logger("BoolTopicCondition"), "target value should be %d", target_value);
      return true;
    }
    else
    {
