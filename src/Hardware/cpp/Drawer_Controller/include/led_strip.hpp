...
#define MAX_NUM_OF_LED_MODES_IN_QUEUE 2
    ...
    *********************************************************************************************************/
    ...
    void add_led_strip_mode_to_queue(robast_can_msgs::CanMessage can_message)
    {
        ...
        xQueueSend(led_strip.led_target_settings_queue, &led_target_settings, 0); // setting the third argument to 0 makes sure, filling the queue wont block, if the queue is already full
    ...
    {
        xQueueReceive(led_strip.led_target_settings_queue, &led_strip_target_settings, 0); // Remove the last target led settings from queue
        uint8_t led_modes_in_queue = uxQueueMessagesWaiting(led_strip.led_target_settings_queue);

        // In case there is another target led setting waiting in the queue, get it and start it
        if (led_modes_in_queue >= 1)
        {
            xQueuePeek(led_strip.led_target_settings_queue, &led_strip_target_settings, 0); // Get the current target led settings from queue without removing it
            select_led_strip_mode();
        }
    }
    ...
    void led_standard_mode()
    {
        ...
        {
            leds[i] = CRGB(led_strip_target_settings.led_target_red, led_strip_target_settings.led_target_green, led_strip_target_settings.led_target_blue);
        }
    ...
    void led_closing_drawer_mode()
    {
        ...
        if ((middle_led - running_led_offset_from_middle) >= 0 - num_of_led_shadows)
        {
            for (int i = 0; i < led_strip.num_leds; i++)
            {
                ...
                {
                    leds[i] = CRGB(led_strip_target_settings.led_target_red / 2, led_strip_target_settings.led_target_green / 2, led_strip_target_settings.led_target_blue / 2);
                }
                // Create a shadow of running LED with less brightness
                ...
                {
                    leds[i] = CRGB(led_strip_target_settings.led_target_red / 3, led_strip_target_settings.led_target_green / 3, led_strip_target_settings.led_target_blue / 3);
                }
                // Create a shadow of running LED with less brightness
                ...
                {
                    leds[i] = CRGB(led_strip_target_settings.led_target_red / 4, led_strip_target_settings.led_target_green / 4, led_strip_target_settings.led_target_blue / 4);
                }
                else
                {
                    leds[i] = CRGB(0, 0, 0);
