...
namespace drawer_controller
{
  ...
  {
   public:
    Drawer(uint32_t module_id, uint8_t id) : module_id_{module_id}, id_{id} {};
    ...
    {
      lock_.handle_lock_control();
      lock_.handle_reading_sensors();
      feedback_msg_.reset();
   ...
   private:
    ...
    bool drawer_open_feedback_can_msg_sent_ = false;
    ...
    void create_drawer_feedback_can_msg()
    {
      ...
      can_signals_drawer_feedback.at(CAN_SIGNAL_DRAWER_ID).set_data(id_);
      ...
      feedback_msg_ = can_msg_drawer_feedback;
    }

    void handle_drawer_is_open_feedback()
    {
      bool is_drawer_open = !lock_.is_endstop_switch_pushed();
      if (lock_.is_drawer_opening_in_progress() && is_drawer_open && !drawer_open_feedback_can_msg_sent_)
      {
        lock_.set_open_lock_current_step(
            false);   // this makes sure the lock automatically closes as soon as the drawer is opened
        drawer_open_feedback_can_msg_sent_ = true;   // makes sure the feedback msg is only sent once
    ...
    {
      bool is_drawer_closed = lock_.is_endstop_switch_pushed() && !lock_.is_lock_switch_pushed();
      if (lock_.is_drawer_opening_in_progress() && is_drawer_closed && drawer_open_feedback_can_msg_sent_)
      {
        lock_.set_drawer_opening_is_in_progress(false);
        drawer_open_feedback_can_msg_sent_ = false;   // reset this flag for the next opening of the drawer
        create_drawer_feedback_can_msg();
      }
    }
