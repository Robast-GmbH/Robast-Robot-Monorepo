#if !defined(DRAWER_STEPPER_CONTROLLER_CAN_HPP)
...
namespace can
{
    ...
    {
        public:
              Can(uint32_t drawer_controller_id, lock::Lock *lock_1, lock::Lock *lock_2,Motor* motor)
            {
        ...
        private:
            ...
            void handle_electrical_drawer_task(robast_can_msgs::CanMessage can_message){
                int speed = can_message.get_can_signals().at(CAN_SIGNAL_DRAWER_GOTO_POSITION).get_data();
                bool stallGuardEnable = can_message.get_can_signals().at(CAN_SIGNAL_DRAWER_STALL_GUARD_ENABLE).get_data() == CAN_DATA_DRAWER_STALL_GUARD_ENABLED ? true : false;

                motor->setSpeed(speed*20,0);
                motor->setStallGuard(stallGuardEnable);
            }
            ...
            void handle_can_msg(robast_can_msgs::CanMessage can_message)
            {
                ...
                if(can_message.get_id() == CAN_ID_ELECTRIC_DRAWER_TASK){
                    if (can_message.get_can_signals().at(CAN_SIGNAL_DRAWER_CONTROLLER_ID).get_data() == this->drawer_controller_id_)
                    {
                        this->handle_electrical_drawer_task(can_message);
                    }
