...
class FeuerplanDoorDetector:
    def __init__(self, image_path:str, model_path:str, distance_from_door_frame:int, confidence_threshold:float) -> None:
        ...
        self.door_detection_model = YOLO(self.model_path)

    def find_obb_doors_in_feuerplan(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        detection_results = self.door_detection_model(source=self.image_path, conf=self.confidence_threshold)
    ...
    def obtain_points_from_detection_results(self) -> DoorDetectionResult:
        ...
        detection_boxes, detection_box_angles = self.find_obb_doors_in_feuerplan()
        door_frame_points = self.process_multiple_boxes(boxes=detection_boxes, angles=detection_box_angles)
        robot_nav_points = [self.perpendicular_projections(point1, point2, self.distance_from_door_frame) for point1, point2 in door_frame_points]
    ...
    def adjust_rotation_and_find_lower_points(self, points:np.ndarray, angle:float, center:np.ndarray) -> List[Tuple[float, float]]:
        """
        ...
        if 150 <= angle <= 210:
            ...
            angle = 90
        R_inv = self.inverse_rotation_matrix(angle)
    ...
    def inverse_rotation_matrix(self, angle:float) -> np.ndarray:
        """
    ...
    def process_multiple_boxes(self, boxes:np.ndarray, angles:np.ndarray) -> List[List[Tuple[float, float]]]:
        """
        ...
        for box, angle in zip(boxes, angles):
            ...
            points_on_a_door = self.adjust_rotation_and_find_lower_points(points, angle, center)
            all_door_points.append(points_on_a_door)
        return all_door_points
    
    def perpendicular_projections(self, point1:Tuple[float,float], point2:Tuple[float,float], distance:int) -> Tuple[Tuple[float,float], Tuple[float,float]]:
        """
    ...
    def visualize_and_save(self, output_path: str) -> None:
        """
        ...
        detection_result = self.obtain_points_from_detection_results()
