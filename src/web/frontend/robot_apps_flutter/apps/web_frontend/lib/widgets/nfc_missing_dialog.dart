...
class NfcMissingDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: WebColors.secondaryBackground,
      title: Text(
        'Nutzer ${identifiers.length != 1 ? 'haben' : 'hat'} keine NFC-ID',
        style: const TextStyle(color: WebColors.primaryText, fontSize: 28),
      ),
      content: Text(
        ...
        style: const TextStyle(color: WebColors.secondaryText, fontSize: 24),
      ),
      actions: [
        TextButton(
          onPressed: () {
          ...
          child: const Text('OK', style: TextStyle(color: WebColors.secondaryText, fontSize: 24)),
        ),
      ],
    );
  }
