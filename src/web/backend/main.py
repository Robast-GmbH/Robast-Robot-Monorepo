...
def update_nfc_code(user_id: str,
                    robot_name="",
                    db: Session = Depends(get_db)):
    db_robot = crud.get_robot(db=db, robot_name=robot_name)
    db_user = crud.get_user(db, user_id)
    task_id = crud.create_task(db,
                               db_robot.robot_name,
                               db_robot.fleet_name,
                               user_id)
    step = 1
    crud.create_new_user_nfc(db, step, task_id, db_user.id)
    action = templates.create_new_user(step=step,
                                       user_id=db_user.id)
    ...
    answer = sender.post(url=config["FLEETMANAGEMENT_ADDRESS"]+"/task",
...
def update_nfc_code_in_db(user_id: str,
                          nfc_code: str = "",
                          db: Session = Depends(get_db)):
    crud.create_nfc_code(db=db,
                         user_id=user_id,
                         nfc_code=nfc_code)
    return
...
def login(user_credentials: schemas.UserLogin, db: Session = Depends(get_db)):
    user = crud.get_users_login(db, user_credentials)
    if user is None:
...
def create_task(task: schemas.Task,
                ...
                user_id: int,
                db: Session = Depends(get_db)):
    task_id = crud.create_task(db=db,
...
def get_next_task(robot_name: str, db: Session = Depends(get_db)):
    ...
    db_robot = crud.get_robot(db=db, robot_name=robot_name)
    crud.update_task(db=db,
                     task_id=next_task.task_id,
                     robot_name=db_robot.robot_name,
                     fleet_name=db_robot.fleet_name)

    actions = templates.create_action_list(db=db,
                                           robot_name=robot_name,
                                           actions=next_task.actions)
    
    task = templates.create_task(db=db,
                                 robot_name=robot_name,
                                 task_id=next_task.task_id,
                                 action=actions)
...
def pause_robot(robot_name: str, fleet_name: str, pause: bool):
    headers = {"Content-Type": "application/json"}
    sender = requests.Session() 
    ...
    if pause:
        answer = sender.post(url=config[
            "FLEETMANAGEMENT_ADDRESS"]+"/settings/navigation/pause",
                             json=json.dumps(message),
                             headers=headers,
                             verify=False)
    else:
        answer = sender.post(url=config[
            "FLEETMANAGEMENT_ADDRESS"]+"/settings/navigation/resume",
                             json=json.dumps(message),
                             headers=headers,
                             verify=False)
...
def set_loop(task: schemas.Task,
             user_id: int,
             force_start: bool,
             db: Session = Depends(get_db)):
    if not all(action.type == 
               schemas.ActionType.NAVIGATION for action in task.actions):
        raise HTTPException(status_code=404,
                            detail="patrol task can only consists of navigate actions")
    ...
    if force_start:
        ...
        sender = requests.Session() 
        answer = sender.post(url=config["FLEETMANAGEMENT_ADDRESS"]+"/task",
                             data=json.dumps(templates.json_task(task)),
                             headers=headers,
                             verify=False)
...
def clear_module_status(robot_name: str,
                        db: Session = Depends(get_db)):
...
def drawer_actions_done(robot_name: str, fleet_name: str):

    message = {"robot_name": robot_name, "fleet_name": fleet_name}
    headers = {"Content-Type": "application/json"}
    sender = requests.Session() 
    answer = sender.post(url=config[
        "FLEETMANAGEMENT_ADDRESS"]+"/settings/drawer/completed",
...
def open_drawer(robot_name: str,
    ...
    crud.create_drawer_action(db,
                              ...
                              module.module_id,
                              module.drawer_id,
                              locked_for=restricted_for_user)

    action = templates.create_drawer_action(step,
                                            module.drawer_id,
                                            module.module_id,
                                            db_module.type == 
                                            models.DrawerSlideTypes.ELECTRICAL,
                                            restricted_for_user,
                                            False)
    if action == "robot":
...
def close_drawer(robot_name: str,
    ...
    sender = requests.Session()
    answer = sender.post(url=config[
        "FLEETMANAGEMENT_ADDRESS"]+"/settings/drawer/close",
                         data=json.dumps(message),
                         headers=headers)
...
def reset_drawer(robot_name: str, db: Session = Depends(get_db)):
    ...
    sender = requests.Session()
    answer = sender.post(url=config[
        "FLEETMANAGEMENT_ADDRESS"]+"/settings/drawer/reset",
                         data=json.dumps(message),
                         headers=headers)
    ...
    if answer.status_code != 200:
        raise HTTPException(status_code=answer.status_code,
                            detail=answer.reason)
    db_modules = crud.get_modules(db=db,
                                  robot_name=robot_name)
    for module in db_modules:
        mod = schemas.UpdateModule(module_id=module.module_id,
                                   drawer_id=module.drawer_id,
                                   robot_name=robot_name,
                                   status="Closed")

        crud.set_module_status(db=db,
                               module=mod)
    return
