...
def get_nfc_codes(db: Session, user_id: int) -> [models.Authentication]:
    return db.query(models.Authentication).filter(
                        models.Authentication.id == user_id,
                        models.Authentication.enabled is True).all()


def get_nfc_code(db: Session, user_id) -> str:
    return db.query(models.Authentication.nfc_code).filter(
        models.Authentication.id == user_id,
        models.Authentication.enabled is True).first()
...
def get_users_login(db: Session, userCredentials: schemas.UserLogin):
    return db.query(models.User).filter(
        models.User.name == userCredentials.name,
        models.User.hashed_password == userCredentials.hashed_password).first()
...
def tasks_queue(db: Session) -> list[models.Task]:
    subquery = db.query(models.Action.task_id).filter(
        models.Action.finished is False).all()
    active_tasks = list(map(lambda x: x[0], subquery))
    queue = db.query(models.Task).filter(
        models.Task.id.in_(active_tasks)).order_by(models.Task.id).all()
    return queue 
...
def create_drawer_action(db: Session,
                         step: int,
                         task_id: str,
                         module_id: int,
                         drawer_id: int,
    ...
    db_action = create_base_action(db,
                                   task_id,
                                   step,
                                   schemas.ActionType.DRAWER)
    ...
    for user_id in locked_for:
        db_drawer_lock = models.DrawerLock(active=False,
                                           user_id=user_id,
                                           unit_id=unit_id)
...
def create_navigation_action(db: Session,
                             step: int,
                             task_id: str,
                             x: float,
                             y: float,
    ...
    db_action = create_base_action(db,
                                   task_id,
                                   step,
                                   schemas.ActionType.NAVIGATION)

    db_nav_action = models.NavigationAction(id=db_action.id,
                                            x_pose=x,
                                            y_pose=y,
                                            yaw_pose=yaw) 
...
def create_new_user_nfc(db: Session, step: int, task_id: str, user_id: int):
    db_action = create_base_action(db,
                                   task_id,
                                   step,
                                   schemas.ActionType.NEW_USER)
...
def get_actions(task_id: str, db: Session) -> [models.Action]:
    return db.query(models.Action).filter(
        models.Action.task_id == task_id).all()


def get_action(action_id: int, db: Session) -> models.Action:
    return db.query(models.Action).filter(
        models.Action.id == action_id).first()


def get_navigation_full_action(action_id: int, db: Session):
    return db.query(models.Action, models.NavigationAction).join(
        models.Action.id == models.NavigationAction.id).filter(
            models.NavigationAction.id == action_id).first()


def get_drawer_full_action(action_id: int, db: Session):
    return db.query(models.Action, models.DrawerAction).join(
        models.Action.id == models.DrawerAction.id).filter(
            models.DrawerAction.id == action_id).first()


def get_user_full_action(action_id: int, db: Session):
    return db.query(models.Action, models.NewUserAction).join(
        models.Action.id == models.NewUserAction.id).filter(
            models.NewUserAction.id == action_id).first()


def get_navigation_action(action_id: int, db: Session):
    return db.query(models.NavigationAction).filter(
        models.NavigationAction.id == action_id).first()


def get_drawer_action(action_id: int, db: Session):
    return db.query(models.DrawerAction).filter(
        models.DrawerAction.id == action_id).first()


def get_user_action(action_id: int, db: Session):
    return db.query(models.NewUserAction).filter(
        models.NewUserAction.id == action_id).first()
...
def delete_action(db: Session, action_id):
    ...
    db.delete(db_action)
    db.commit()
    db_sub_action = db.query(models.DrawerAction).filter(
        models.DrawerAction.id == action_id)
    if db_sub_action is None:
        db_sub_action = db.query(models.NavigationAction).filter(
            models.NavigationAction.id == action_id)
    elif db_sub_action is None:
        db_sub_action = db.query(models.NewUserAction).filter(
            models.NewUserAction.id == action_id)
...
def get_next_task(db: Session, robot_name: str):
...
def get_full_task( db: Session,task_id: int) -> schemas.Task:
    ...
    for db_action in db_actions:
        ...
        #   locked_for= [db_drawer_action.target_user_id]
            ...
            drawer_action = schemas.Drawer(drawer_id=drawer_id,
                                           module_id=module_id,
                                           locked_for=[])
...
def get_modules(db: Session, robot_name: str) -> [models.Module]:
    return db.query(models.Module).filter(
        robot_name == models.Module.robot_name == robot_name).order_by(
...
def get_module_by_module_and_drawer_id(db: Session,
                                       module_id: int,
                                       drawer_id: int) -> models.Module:
    return db.query(
        models.Module).filter(
            models.Module.module_id == module_id and 
            models.Module.drawer_id == drawer_id).first()


def get_drawer(db: Session,
               robot_name: str,
               module_id: int,
               drawer_id: int) -> models.Module:
    drawer = db.query(models.Module).filter(
        models.Module.robot_name == robot_name, 
...
def set_drawers_to_close(db: Session, robot_name: str):
    db.query(models.Module).filter(
        models.Module.robot_name == robot_name).update(
            {models.Module.status: "Closed"})
    db.commit()
    return


def get_unit_id(db: Session, module_id: int, drawer_id: int) -> int:
    return db.query(models.Module.unit_id).filter(
        models.Module.drawer_id == drawer_id,
        models.Module.module_id == module_id).first()[0]


def get_module_by_unit_id(db: Session, unit_id: int):
    return db.query(models.Module).filter(
        models.Module.unit_id == unit_id).first()


def set_module(db: Session,
               module: schemas.Module,
               override: bool) -> models.Module:
    db_module = get_drawer(db=db,
...
def set_module_status(db: Session,
                      module: schemas.UpdateModule) -> models.Module:
    db_module = get_drawer(db=db,
                           robot_name=module.robot_name,
                           module_id=module.module_id,
                           drawer_id=module.drawer_id)
...
def remove_unlocked_state(db: Session):
    db.query(models.Module).filter(models.Module.status == "Unlocked").update(
        {models.Module.status: "Closed"})
