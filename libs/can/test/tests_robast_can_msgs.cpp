...
SCENARIO("Test class creation of CanSignal, CanMessage, CanDb and CanFrame", "[robast_can_msgs]")
{
    GIVEN("A CAN msg_id and dlc as well as data for a drawer_id, LED_red, LED_green, LED_blue, LED_brightness, LED_mode")
    {
        ...
        WHEN("Creating the CanSignal classes")
        {
            ...
            robast_can_msgs::CanSignal can_signal_module_id = robast_can_msgs::CanSignal(bit_start_module_id, bit_length_module_id, data_module_id);
            ...
            WHEN("Creating the CanMessage class")
            {
                ...
                THEN("The created CanMessage class should encapsulate the data correctly")
                {
                    ...
                    REQUIRE(can_message.get_can_signals()[CAN_SIGNAL_MODULE_ID].get_bit_start() == can_signal_module_id.get_bit_start());
                    REQUIRE(can_message.get_can_signals()[CAN_SIGNAL_MODULE_ID].get_bit_length() == can_signal_module_id.get_bit_length());
                    REQUIRE(can_message.get_can_signals()[CAN_SIGNAL_MODULE_ID].get_data() == can_signal_module_id.get_data());
                    REQUIRE(can_message.get_can_signals()[CAN_SIGNAL_DRAWER_ID].get_bit_start() == can_signal_drawer_id.get_bit_start());
                    REQUIRE(can_message.get_can_signals()[CAN_SIGNAL_DRAWER_ID].get_bit_length() == can_signal_drawer_id.get_bit_length());
                ...
                WHEN("Creating the CanDb class")
                {
                    ...
                    THEN("The created CanDb class should contain the correct id, dlc and CanSignals with the correct bit_start and bit_length and data should be default 0.")
                    {
                        ...
                        REQUIRE(can_db.can_messages[CAN_MSG_DRAWER_LED].get_can_signals()[CAN_SIGNAL_MODULE_ID].get_bit_start() == can_signal_module_id.get_bit_start());
                        REQUIRE(can_db.can_messages[CAN_MSG_DRAWER_LED].get_can_signals()[CAN_SIGNAL_MODULE_ID].get_bit_length() == can_signal_module_id.get_bit_length());
                        REQUIRE(can_db.can_messages[CAN_MSG_DRAWER_LED].get_can_signals()[CAN_SIGNAL_MODULE_ID].get_data() == 0);
                        REQUIRE(can_db.can_messages[CAN_MSG_DRAWER_LED].get_can_signals()[CAN_SIGNAL_DRAWER_ID].get_bit_start() == can_signal_drawer_id.get_bit_start());
                        REQUIRE(can_db.can_messages[CAN_MSG_DRAWER_LED].get_can_signals()[CAN_SIGNAL_DRAWER_ID].get_bit_length() == can_signal_drawer_id.get_bit_length());
...
SCENARIO("Test CAN helper functions", "[robast_can_msgs]")
{
    GIVEN("A CAN msg_id and dlc as well as data for a drawer_id, LED_red, LED_green, LED_blue")
    {
        ...
        {
            uint64_t u64_can_data_unlock = robast_can_msgs::join_together_CAN_data_bytes_from_array(u8_can_data_unlock, dlc_unlock);
        ...
        {
            std::optional<robast_can_msgs::CanMessage> decoded_can_message_drawer_unlock = robast_can_msgs::decode_can_message(msg_id_unlock, u8_can_data_unlock, dlc_unlock, can_db.can_messages);
