...
#define MODULE_CONFIG_ID_DRAWER_MOVING_IN_DECELERATION_DISTANCE \
  4   // distance to the target position to start deceleration (max 255)
#define MODULE_CONFIG_ID_DRAWER_MOVING_IN_FINAL_HOMING_DISTANCE \
  5   // the end of the distance when moving in where speed is super slow
#define MODULE_CONFIG_ID_DRAWER_MOVING_OUT_DECELERATION_DISTANCE \
  6   // distance to the target position to start deceleration (max 255)
#define MODULE_CONFIG_ID_DRAWER_PUSH_IN_AUTO_CLOSE_SPEED 7
#define MODULE_CONFIG_ID_DRAWER_PUSH_IN_AUTO_CLOSE_STALL_GUARD_VALUE \
  8   // the higher the value the more sensitive the stall guard is
#define MODULE_CONFIG_ID_DRAWER_PUSH_IN_WAIT_TIME_AFTER_STALL_GUARD_TRIGGERED_IN_MS 9
  ...
  template <>
  struct ModuleConfigDataType<MODULE_CONFIG_ID_DRAWER_PUSH_IN_WAIT_TIME_AFTER_STALL_GUARD_TRIGGERED_IN_MS>
  {
    using type = uint32_t;
